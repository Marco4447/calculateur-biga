...
with st.sidebar:
    ...
    poids_paton_voulu = st.number_input("Poids d'un pâton fini (g)", value=1000, step=10)
    ...
    # Règle : 44% d'eau sur la farine de la biga
    pct_eau_biga = 44

    st.divider()
    st.header("🌡️ Températures")
    t_amb = st.number_input("Temp. Ambiante (°C)", value=22)
    t_far = st.number_input("Temp. Farine (°C)", value=20)
    friction_calculee = 15.0  # Valeur par défaut simplifiée

# 3. MOTEUR DE CALCUL INVERSÉ
# On calcule la farine totale pour que (Farine + Eau + Sel + Huile + Malt + Levure 1%) = Poids_Fini
ratio_total = 1 + (hydra_totale / 100) + (sel_pct / 100) + (huile_pct / 100) + (malt_pct / 100) + 0.01
farine_totale = (nb_patons * poids_paton_voulu) / ratio_total
...
# 4. AFFICHAGE
st.markdown(f"### 📊 Pour {nb_patons} pâton(s) de {poids_paton_voulu}g (Poids fini)")
